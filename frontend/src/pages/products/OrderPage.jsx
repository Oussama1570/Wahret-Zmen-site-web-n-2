...
import { useRemoveProductFromOrderMutation } from "../../redux/features/orders/ordersApi";
...
const OrderPage = () => {
  ...
  const handleDeleteProduct = async (orderId, productId, colorName, maxQuantity) => {
    const productKey = `${productId}|${colorName}`;
  
    const { value: quantityToRemove } = await Swal.fire({
      title: "Remove Quantity",
      input: "number",
      inputLabel: `You have ${maxQuantity} in your order. Enter quantity to remove:`,
      inputAttributes: {
        min: 1,
        max: maxQuantity,
        step: 1,
    ...
    try {
      ...
      Swal.fire("Removed!", `${quantityToRemove} item(s) removed from order.`, "success");
      refetch();
    } catch (error) {
 ...
 <button
  onClick={() =>
    handleDeleteProduct(order._id, product.productId._id, product.color.colorName, product.quantity)
  }
